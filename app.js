...
(function () {
  ...
  function formatPrice(n) {
    const rub = Math.round(Number(n) * eurRate);
  ...
  function render() {
    ...
    if (fetchFailed) {
      ...
      pagerInput.value = ''; pagerTotal.textContent = '';
    ...
    pagerInput.value = currentPage; pagerInput.max = totalPages; pagerTotal.textContent = totalPages;
  ...
  pagerInput.addEventListener('change', () => {
    const totalPages = Math.max(1, Math.ceil(getFiltered().length / PAGE_SIZE));
    let target = parseInt(pagerInput.value, 10);
    if (!target || target < 1) target = 1;
    if (target > totalPages) target = totalPages;
    currentPage = target;
    renderPaged();
  });
  pagerInput.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') { e.preventDefault(); pagerInput.blur(); }
  });
  ...
  // Курс евро для перевода цен в рубли на сайте. Хранится в таблице
  // settings в Supabase и управляется из админки (кнопка "Применить курс").
  // Кэшируем так же, как каталог — чтобы при сбое сети сайт не остался
  // без цен вовсе, а показывал последний известный курс.
  const RATE_CACHE_KEY = 'podium_eur_rate_v1';
  let eurRate = 100; // запасное значение на случай, если ничего не найдётся
  ...
  function readRateCache() {
    try {
      ...
      return (parsed && parsed.rate > 0) ? parsed.rate : null;
    } catch (e) {
      return null;
    }
  }
  ...
  function writeRateCache(rate) {
    try {
      localStorage.setItem(RATE_CACHE_KEY, JSON.stringify({ rate, savedAt: Date.now() }));
    } catch (e) {}
  }

  function fetchRate() {
    return fetchWithTimeout(`${SUPABASE_URL}/rest/v1/settings?select=value&key=eq.eur_rate`, {
      headers: {
      ...
      const val = rows && rows[0] && Number(rows[0].value);
      if (!val || val <= 0) throw new Error('Bad rate value');
      return val;
    });
  }

  const cachedRate = readRateCache();
  if (cachedRate) eurRate = cachedRate;
  ...
  fetchRate()
    ...
    .catch(err => {
      console.warn('Не удалось получить курс евро, используем последний известный:', err);
    });
